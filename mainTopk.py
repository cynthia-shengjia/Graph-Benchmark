...
from tools.procedure import train_batch_topk as train_batch_topk
...
LOSSES = {
    "PreAtK": optimizer.optim_PreAtK.TopKOptimizer
}
...
user_all_pos        = data["user_all_pos"]
...
loss_func = LOSSES[world.config["loss"]](model=model, node_num = node_num, config=world.config)
...
for epoch in range(world.config["epochs"]):
    ...
    loss = train_batch_topk(model, loss_func, train_pos, x, interaction_tensor, user_all_pos, world.config["batch_size"])
